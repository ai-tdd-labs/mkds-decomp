// PURPOSE: Searches backward through connected checkpoint paths for a matching course point.
...
int StructRC_SomeFn_1(DriverStatus *driverStatus,
                      int *value,
                      VecFx32 *position,
                      int cpatIndex,
                      int cpoiIndex,
...
{
    s16 sectionData =
        data_02175620->cpoiEntries[cpoiIndex].sectionData2;
    int result;
    int linkIndex;
    NkmCPAT *cpat;
    ...
    if (sectionData != -1) {
        ...
        for (linkIndex = 0; linkIndex < 3; linkIndex++) {
            s8 previousCpat = cpat->previousGroups[linkIndex];

            if (previousCpat != -1) {
                result = StructRC_SomeFn_1(
                    driverStatus, value, position, (u16)previousCpat,
                    (u16)(data_02175620->cpatEntries[(u16)previousCpat].pointStart +
                          data_02175620->cpatEntries[(u16)previousCpat].pointCount - 1),
                    remainingDepth - 1);
                if (result != -1) {
                    return result;
                }
    ...
    return StructRC_SomeFn_1(driverStatus, value, position, cpatIndex,
