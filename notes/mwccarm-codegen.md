<!-- Geïmporteerd uit sm64ds-decomp/notes/mwccarm-codegen.md (MIT, tangosdev). Idiomen transfereren
     want zelfde compilerfamilie mwccarm 1.2. Verifieer elke les tegen MKDS. -->
...
ModelBlock material-loop family (11 leden, 0x48 elk, `0x02012ffc`-`0x020132cc`, cracked
2026-07-14): `for (i = 0; i < model->numMaterials; i++) callee(model, i, arg1);` waarbij
`numMaterials` een `unsigned char` op struct-offset `0x18` is. Twee vereisten om byte-exact
te krijgen:
- de loopteller moet `unsigned int i` zijn, NIET `int i` -- met een signed teller emit mwccarm
  `pople`/`bxle`/`blt` (signed), terwijl de ROM `popls`/`bxls`/`blo` (unsigned) gebruikt, ook al
  is `numMaterials` na integer-promotie sowieso non-negatief. Het teken van de VERGELIJKING volgt
  het teken van de LOOPVARIABELE, niet dat van het (kleinere) veld waarmee ze vergeleken wordt.
- `model->numMaterials` wordt ELKE iteratie opnieuw geladen (geen registerhoisting), want de
  callee krijgt `model` als pointer en kan het veld theoretisch wijzigen -- normaal mwccarm-gedrag
  voor een niet-const pointer-parameter, dus schrijf de C ook als een simpele `for`-loop met
  member-access in de conditie (niet met een lokale `count`-variabele vooraf gecached).
Alle 11 leden zijn identieke shells die alleen verschillen in functienaam + callee-naam; template
zie `ai/scratch/fam7_*.c`.
