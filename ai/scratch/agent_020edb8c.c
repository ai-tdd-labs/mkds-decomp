// PURPOSE: Builds a travel direction and updates the remaining distance.
// The object probably supplies the current movement and facing vectors.
// The scalar arguments adjust the distance and the alternate direction.
...
fx32 func_020edb8c(MovementObject *object, fx32 distance, fx32 factor,
                    fx32 scale, VecFx32 *out)
{
    ...
    if (object->use_alternate_080 != 0) {
        distance -= length;
        VecFx32_Subtract(&object->movement_06c,
                         &object->source_010->reference_1fc, &direction);
        VecFx32_AddScale1(scale, &object->direction_044, &direction,
                          &direction);
        direction.x = -direction.x;
        direction.y = -direction.y;
        direction.z = -direction.z;
    } else {
        direction.x = -object->direction_044.x;
        direction.y = -object->direction_044.y;
        direction.z = -object->direction_044.z;
    ...
    /* The SDK only normalizes vectors whose squared length is large enough. */
