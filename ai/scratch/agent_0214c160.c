// PURPOSE: Copies palette data to the top screen palette memory.
...
void CopyPaletteTop(const void *src, u32 offset, u32 size)
{
    int dmaNo = data_0216f074;

    if (dmaNo != -1 && size > 0x1c) {
        MI_DmaCopy16(dmaNo, src, (void *)(offset + 0x05000000), size);
    } else {
        CopyMemory16(src, (void *)(offset + 0x05000000), size);
    }
}
