...
void func_0202dc68(int recordIndex, void *output)
{
    if (func_0202dd80(recordIndex) == 0) {
        ...
        volatile GhostRecordContext *ctx = data_02174e44;
        int extraIndex = recordIndex - ctx->extraRecordStartIndex;
        u8 *blocks = ctx->extraRecordBlocks;
        u8 blockId = (u8)ctx->extraRecordBlockIds[extraIndex];

        func_020616c0((RecordEntry *)(blocks + blockId * 0x230), output);
    }
}
